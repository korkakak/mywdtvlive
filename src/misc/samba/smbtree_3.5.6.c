...
static int smb_mount(char *name, char *passwd, char *pcname, char *ip, char *dir)
{
	...
	if(access(filename, 0) != 0) { // pc root dir don't exist.
		snprintf(cmdline, sizeof(cmdline), "mkdir \"%s/%s\" -p", CONF_SAMBA_MOUNT_POINT, pcname);	
		ret = system(cmdline);
		if(ret == -1) {
			return ret;
		}
	...
	if(access(filename, 0) != 0) { // shared dir don't exist.
		memset(cmdline, 0, sizeof(cmdline));
		snprintf(cmdline, sizeof(cmdline), "mkdir \"%s/%s/%s\" -p", CONF_SAMBA_MOUNT_POINT, pcname, dir);	
		ret = system(cmdline);
		if(ret == -1) {
