...
/* For 802.3 test */
#define	PROC_ROOT		"r8169"
#define PROC_NORMAL		"normal"
#define PROC_TEST_1		"test1"
#define PROC_TEST_2		"test2"
#define PROC_TEST_3		"test3"
#define PROC_TEST_4		"test4"
#define PROC_MODE_QUERY		"mode_query"
...
static	int proc_read_generic(char *page, char **start, off_t off,
			  		int count, int *eof, void *data)
{
	int	ret;
	struct priv_data *pdata = data;
	struct net_device *dev = pci_get_drvdata(pdata->pdev);
	struct rtl8169_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;

	ret = mdio_read(ioaddr, 0x9);
	printk("read 0x9 return = 0x%04x\n", ret);
	ret = sprintf( page + off, "0x%x \n", ret );
	*eof = 1;
	return ret;
}

static	int proc_write_generic(struct file *file, const char __user *buffer,
			   		unsigned long count, void *data)
{
	int value;
	struct priv_data *pdata = data;
	int reg_value = pdata->reg_value;
	struct net_device *dev = pci_get_drvdata(pdata->pdev);
	struct rtl8169_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;

	printk("write bit 15:13 value =0x%04x\n", reg_value);
	value = (reg_value << 13);
	printk("value = 0x%04x\n", value);
	mdio_write(ioaddr, 0x09, value);
	return count;
}


static int r8169_init_proc(struct pci_dev *pdev)
{
	struct	proc_dir_entry	*ent;

	printk("r8169 init proc\n");
	...
	if ((ent = create_proc_entry( PROC_NORMAL, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
		ent->write_proc = proc_write_generic;
		pdata = (struct priv_data *)kmalloc(1, GFP_KERNEL);
		pdata->reg_value = 0x0;
		pdata->pdev = pdev;
		ent->data = (void *)pdata;
	}

	if ((ent = create_proc_entry( PROC_TEST_1, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
		ent->write_proc = proc_write_generic;
	...
	if ((ent = create_proc_entry( PROC_TEST_2, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
		ent->write_proc = proc_write_generic;
	...
	if ((ent = create_proc_entry( PROC_TEST_3, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
		ent->write_proc = proc_write_generic;
	...
	if ((ent = create_proc_entry( PROC_TEST_4, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
		ent->write_proc = proc_write_generic;
	...
	if ((ent = create_proc_entry( PROC_MODE_QUERY, S_IFREG | 0222, proc_ent_root )) == NULL) 
		return -EINVAL;
	else {
		struct priv_data *pdata;
...
static void  r8169_cleanup_proc(void) 
{
	printk("r8169_cleanup_proc\n");
	if (proc_ent_root) {
		remove_proc_entry( PROC_NORMAL, proc_ent_root );
		remove_proc_entry( PROC_MODE_QUERY, proc_ent_root );
		remove_proc_entry( PROC_TEST_1, proc_ent_root );
		remove_proc_entry( PROC_TEST_2, proc_ent_root );
		remove_proc_entry( PROC_TEST_3, proc_ent_root );
		remove_proc_entry( PROC_TEST_4, proc_ent_root );
...
rtl8169_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	printk("%s,%d:  r8169 init proc\n",__FUNCTION__,__LINE__);
	r8169_init_proc(pdev);
	printk(KERN_INFO "%s: This product is covered by one or more of the following patents: US5,307,459, US5,434,8.\n", MODULENAME);
...
static int rtl8169_open(struct net_device *dev)
{
	...
	retval = request_irq(dev->irq, rtl8169_interrupt, IRQF_SHARED, dev->name, dev);
				...
				unsigned int align)
{
	...
	skb_reserve(skb, (align - 1) & (uintptr_t)skb->data);
	*sk_buff = skb;
