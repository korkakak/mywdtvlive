...
static void hsata_bmdma_start( struct ata_queued_cmd *qc )
{
	...
	/*
         * We have to manually control our host controller's DMA Tx.  This
	 * is because the controller would otherwise send a data FIS out
	 * after each LLI block of data.  This is inefficient.  If we 
	 * delimit the entire Tx data by enabling and disabling it's DMA
	 * channel, it can send out larger (up to 8K) data FISes.
	 * See also the hsata_dma_isr for clearing of this bit when the
	 * entire LLI table has been processed by the DMAC.
	 */
	if (dir == DMA_TO_DEVICE) { 
	   	/* Tx Burst length */
