...
_func_enter_;
		...
		(check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE) )	
	{
		...
		if((_memcmp(&pmlmepriv->cur_network.network.MacAddress, bssid, 6))==_TRUE)
		{		
...
_func_enter_;
		...
		(check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE) == _TRUE) )	
	{
		...
		{		
			...
			if((check_fwstate(pmlmepriv, WIFI_STATION_STATE) == _FALSE))
			{
				RT_TRACE(_module_rtl871x_ioctl_set_c_,_drv_err_,("Set SSID ist the same ssid, fw_state=0x%x\n.", pmlmepriv->fw_state));
...
_func_enter_;
	...
	if(*pold_state != networktype)
	{
		...
		*pold_state = networktype;
		...
		pmlmepriv->fw_state &= ~(WIFI_STATION_STATE|WIFI_AP_STATE|WIFI_ADHOC_STATE|WIFI_ADHOC_MASTER_STATE|WIFI_AP_STATE);
		...
		switch(networktype)
		{
			...
			case Ndis802_11APMode:
				pmlmepriv->fw_state |= WIFI_AP_STATE;
