				...
				char *start, char *stop  )
{
	...
	/* Steven Kuo: Copy signal strength to qual, we will convert it to signal strength value */
	iwe.u.qual.qual = (u8)pnetwork->network.Rssi;  // signal quality
...
{	
	...
	{
		printk("wpa_set_auth_algs, AUTH_ALG_SHARED_KEY\n");
...
static int r8711_wx_set_freq(struct net_device *dev, 
			     ...
			     union iwreq_data *wrqu, char *extra)
{
	RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("r8711_wx_set_freq\n"));
...
#endif			
				printk("scan_q is empty, set ssid to check if scanning again!\n");
