...
{
	struct cmd_obj *pcmd;
	unsigned int wr_sz, *pcmdbuf, *prspbuf;
...
_next:
		...
		if(pcmd)//if pcmd != NULL, the cmd will be handled by f/w
		{
			...
			pcmd->cmdsz = _RND8( (pcmd->cmdsz));//_RND8	

			wr_sz = TXDESC_SIZE + 8 + pcmd->cmdsz;		 	
			...
			*pcmdbuf = cpu_to_le32((pcmd->cmdsz&0x0000ffff) | (pcmd->cmdcode << 16) | (pcmdpriv->cmd_seq <<24));
