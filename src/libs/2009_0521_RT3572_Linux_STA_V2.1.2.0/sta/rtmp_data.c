	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
		...
		if (RTMPCheckWAIframe(pAd, pRxBlk->pData, pRxBlk->DataSize))
		{
			Indicate_Legacy_Packet(pAd, pRxBlk, FromWhichBSSID);
			return;
		}			
	...
	IN	RX_BLK			*pRxBlk)
{
	...
	{	
		...
		{
			pRxIv = pAd->StaCfg.rx_iv;
			pWpiKey = pAd->StaCfg.GTK;			
		}

		//if (!RTMPSoftDecryptSMS4(pAd, (PUCHAR)pHeader, pRxBlk->pData, pRxBlk->DataSize, pWpiKey, pRxIv))
		if (!RTMPSoftDecryptSMS4(pAd, 
								 (PUCHAR)pHeader, 
								 pRxBlk->pData, 
								 pRxBlk->DataSize, 
								 pWpiKey, 
								 pRxIv, 
								 RX_BLK_TEST_FLAG(pRxBlk, fRX_AMPDU)))
		{
			// release packet			
			RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_FAILURE);
			return;
		...
		// skip IV header and subtract MIC length
		pRxBlk->pData += (2 + LEN_TX_IV);
		pRxBlk->DataSize -= (2 + LEN_TX_IV + LEN_WPI_MIC);
	...
	IN	TX_BLK			*pTxBlk)
{
	...
	else if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bWPIDataFrame))	
	{
		// WPI data packet MUST be encrypted.
	...
	IN	TX_BLK			*pTxBlk)
{
	...
	while(pTxBlk->TxPacketList.Head)
	{
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bWPIDataFrame))
		{				
			pTxBlk->pPacket = duplicate_pkt_with_WPI(pAd, pTxBlk->pPacket, 0, LEN_WPI_MIC);
		...
		*(pHeaderBufPtr+1) = 0;
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bWPIDataFrame))
		{			
			// insert WPI-SMS4 IV header to MPDU header
			RTMPInsertWpiIvHdr(pAd,
							   pTxBlk->pMacEntry->usk_id,
							   pTxBlk->pWpiDataPn,
							   pHeaderBufPtr);		
			pHeaderBufPtr += LEN_WPI_IV_HDR;
			pTxBlk->MpduHeaderLen += LEN_WPI_IV_HDR;
			...
			RTMPSoftEncryptSMS4(pAd, 
								(PUCHAR)pHeader_802_11, 
								pTxBlk->pSrcBufData, 
								pTxBlk->SrcBufLen, 
								pTxBlk->pMacEntry->usk_id,
								pTxBlk->pMacEntry->PTK, 
								pTxBlk->pWpiDataPn);

			pTxBlk->SrcBufLen += LEN_WPI_MIC;
			pTxBlk->TotalFrameLen += LEN_WPI_MIC;
