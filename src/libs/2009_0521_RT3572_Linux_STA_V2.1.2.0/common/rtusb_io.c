...
//	ULONG 		i;
	...
	if (IS_RT3070(pAd) || IS_RT3071(pAd) || IS_RT3572(pAd))
	{
		...
		RTMPusecDelay(200);
		AsicSendCommandToMcu(pAd, 0x36, 0, 0, 0);
		RTMPusecDelay(10);
	}
#endif // NEW_FW //
	...
	IN	USHORT			length)
{
	...
	if(pAd->bUseEfuse)
	{
		...
		{
			printk("Read from EEPROM Buffer\n");
			NdisMoveMemory(pData, &(pAd->EEPROMImage[Offset]), length);
		}
		else
	...
	IN	USHORT			length)
{
	...
	if(pAd->bUseEfuse)
	{
		if(pAd->bFroceEEPROMBuffer||pAd->bEEPROMFile)
			{
			printk("Write to EEPROM Buffer\n");
			NdisMoveMemory(&(pAd->EEPROMImage[Offset]), pData, length);
			}
		else
