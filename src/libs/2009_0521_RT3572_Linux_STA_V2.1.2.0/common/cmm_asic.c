	...
	if (IS_RT3572(pAd) /*&& (pAd->RfIcType == RFIC_3052)*/)
	{
		for (index = 0; index < NUM_OF_3572_CHNL; index++)
		{
			if (Channel == FreqItems3572[index].Channel)
			{
				...
				RTMP_IO_READ32(pAd, GPIO_CTRL_CFG, &RFValue);
				if  (Channel <= 14)
					RFValue = ((RFValue & 0xFFFF7FFF) | 0x00000080);
				else
					RFValue = (RFValue & 0xFFFF7F7F);
				RTMP_IO_WRITE32(pAd, GPIO_CTRL_CFG, RFValue);
...
#ifndef NEW_FW
#ifdef RT35xx
	if (IS_RT3572(pAd) && (Command == 0x30 || Command == 0x31))
		return FALSE;
#endif // RT35xx //
#endif
#endif // RTMP_MAC_USB //
