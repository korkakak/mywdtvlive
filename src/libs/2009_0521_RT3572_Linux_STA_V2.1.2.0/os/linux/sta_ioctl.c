...
#ifdef RTMP_MAC_USB
#ifdef RT35xx
	if (pAd && IS_RT3572(pAd))
		strncpy(name, "RT3572 Wireless", IFNAMSIZ);
	else
#endif // RT35xx //
                ...
                if (Status == NDIS_STATUS_SUCCESS)
                {
					...
					{						
						if (NdisEqualMemory(pAd->MlmeAux.Bssid, wapi_ukey.Addr, MAC_ADDR_LEN))
						{
							...
							// Initial TX/RX IV
							NdisMoveMemory(pEntry->tx_iv, ASUE_UCAST_PN, LEN_TX_IV);
							NdisMoveMemory(pEntry->rx_iv, AE_UCAST_PN, LEN_TX_IV);
		...
		case OID_802_11_MCAST_KEY_INFO:
            ...
            {                												
                ...
                {                	
                	...
                	NdisMoveMemory(pAd->StaCfg.rx_iv, wapi_mkey.m_tx_iv, 16);
                	NdisMoveMemory(pAd->StaCfg.NMK, wapi_mkey.NMK, 16);

					// Calculate GTK
					RTMPCalculateWapiGTK(pAd, pAd->StaCfg.NMK, pAd->StaCfg.GTK);
    ...
    IN  PSTRING          arg)
{
    if ((strcmp(arg, "WEPAUTO") == 0) || (strcmp(arg, "wepauto") == 0))
        ...
        pAdapter->StaCfg.AuthMode = Ndis802_11AuthModeWAICERT;    
    else if ((strcmp(arg, "WAIPSK") == 0) || (strcmp(arg, "waipsk") == 0))
        pAdapter->StaCfg.AuthMode = Ndis802_11AuthModeWAIPSK;
#endif // WAPI_SUPPORT //
    else
        return FALSE;  
    ...
    IN  PSTRING          arg)
{
    ...
    {
        if ((pAdapter->StaCfg.AuthMode != Ndis802_11AuthModeWAICERT) &&
			(pAdapter->StaCfg.AuthMode != Ndis802_11AuthModeWAIPSK))
            return TRUE;    // do nothing
            
        pAdapter->StaCfg.WepStatus     = Ndis802_11EncryptionSMS4Enabled;
        pAdapter->StaCfg.PairCipher    = Ndis802_11EncryptionSMS4Enabled;
	    pAdapter->StaCfg.GroupCipher   = Ndis802_11EncryptionSMS4Enabled;
    }
#endif // WAPI_SUPPORT //
    else
        return FALSE;
