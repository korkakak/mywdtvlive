...
#ifdef RT35xx
	if (IS_RT3572(pAd))
		RestoreRfICType=pAd->ate.RfIcType;
	else
#endif // RT35xx //
	...
	if (pAd->ate.TxWI.BW == BW_20)
	{
	    ...
	    */
		...
		if (IS_RT3572(pAd))
		{
			...
			ATE_RF_IO_WRITE8_BY_REG_ID(pAd, RF_R24, (UCHAR)pAd->Mlme.CaliBW20RfR24);
//			ATE_RF_IO_WRITE8_BY_REG_ID(pAd, RF_R31, (UCHAR)pAd->Mlme.CaliBW20RfR24);
...
INT Set_ATE_HW_LED_Proc(
	...
	IN PSTRING			arg)
{
	if (IS_RT3572(pAd) && (pAd->ate.RfIcType == RFIC_3052))
	{
...
//				else
				...
				ATEDBGPRINT(RT_DEBUG_TRACE, ("RT35xx: SwitchChannel#%d(RF=%d, Pwr0=%d, Pwr1=%d, %dT), N=0x%02X, K=0x%02X, R=0x%02X\n",
					Channel, 
					pAd->ate.RfIcType, 
					TxPwer,
					TxPwer2,
