...
#else
			{
				zfRxBufferCopy(dev, newBuf, buf, 0, *offset, 12);
				zfRxBufferCopy(dev, newBuf, buf, 12, *offset+12+8, subframeLen-6);
				zfwBufSetSize(dev, newBuf, 12+subframeLen-6);
			}
#endif//#ifndef MAC_OS_USED_FLAG
#endif//#ifdef ZM_ENABLE_NATIVE_WIFI   
            /* Update offset */
