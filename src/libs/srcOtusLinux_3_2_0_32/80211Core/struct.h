...
extern void zfwBufTrace(zdev_t* dev, zbuf_t *buf, s8_t *functionName);
...
#define ZM_CACHED_FRAMEBODY_SIZE   300    
...
#define    ZM_RIFS_TIMER_TIMEOUT      4480          // <Driver time>4480ms <Real time>7s
    ...
    u32_t  additionalAssocIESize;
    u8_t   additionalAssocIE[256];

    u8_t   adhocEnableHT;
    u8_t   adhoc5GHzEnableHT40;
    u8_t   adhocEnableAggregation;
    u8_t   acmFlag; 	
}; //struct zsWlanDevSta
...
#define ZM_CMD_RESET_TO_BOOT   0x94
...
struct zsWlanDev
{
    ...
    /* Monitor mode for customer */
    u8_t                monitorEnable;

    u8_t                NewAPSelAlog;	
    /* Used to record the frquency of the PSP created network */
    u16_t               PSPFrequency;
