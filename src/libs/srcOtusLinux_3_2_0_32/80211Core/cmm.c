...
u16_t zfMmAddHTCapability(zdev_t* dev, zbuf_t* buf, u16_t offset)
{
    ...
    else
    {
        ...
        {
            zmw_buf_writeb(dev, buf, offset++, 0x0C);
            zmw_buf_writeb(dev, buf, offset++, 0x00);
            zmw_buf_writeb(dev, buf, offset++, 0x1B);
            for (i = 3; i < 26; i++)
            {
        ...
        {
            /* zmw_buf_writeh() should handle endian issue. */
            zmw_buf_writeh(dev, buf, offset++, *(u16_t*)(&wd->sta.HTCap.Byte[2]));        
            offset++;
...
u16_t zfMmAddAtherosNHTCapability(zdev_t* dev, zbuf_t* buf, u16_t offset)
{
    u16_t i;

    zmw_get_wlan_dev(dev);     
    
    zmw_buf_writeb(dev, buf, offset++, ZM_WLAN_EID_HT_CAPABILITY);
...
u16_t zfMmAddExtendedHTCapability(zdev_t* dev, zbuf_t* buf, u16_t offset)
{
    ...
    else
    {
        ...
        if ( (wd->frequency > 3000) && (wd->wlanMode == ZM_MODE_IBSS) && (!wd->sta.adhoc5GHzEnableHT40) )
        {
            zmw_buf_writeb(dev, buf, offset++, 0x30);
            zmw_buf_writeb(dev, buf, offset++, 0x00);
            zmw_buf_writeb(dev, buf, offset++, 0x01);
            for (i = 3; i < 22; i++)
            {
...
u16_t zfMmAddAtherosNExtendedHTCapability(zdev_t* dev, zbuf_t* buf, u16_t offset)
{
    u16_t i;

    zmw_get_wlan_dev(dev);     

    zmw_buf_writeb(dev, buf, offset++, ZM_WLAN_EID_EXTENDED_HT_CAPABILITY);

    /* Element Length */
    zmw_buf_writeb(dev, buf, offset++, wd->sta.ExtHTCap.Data.Length);
    ...
    switch (frameType)
    {
        ...
        case ZM_WLAN_FRAME_TYPE_REASOCREQ :
            ...
            if (wd->sta.EnableHT != 0)
            {
                ...
                wd->sta.HTCap.Data.AMPDUParam |= HTCAP_MaxRxAMPDU3;
                if (zfHpCapability(dev) & ZM_HP_CAP_11N_ONE_RX_STREAM)
                {
                    wd->sta.HTCap.Data.MCSSet[1] = 0x00; // MCS 8 ~ 15
                }
                else
                {
            ...
            if ( wd->sta.additionalAssocIESize )
                offset = zfStaAddAdditionalAssocIE(dev, buf, offset);
