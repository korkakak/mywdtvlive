...
//          temp = (S[i] + temp) & 255;
        ...
        xui = (ui&0xfe)+(((ui&0x1)==1)?test[0]:test[1]);
        In = snap[xui];
...
//          temp = (S[i] + temp) & 255;
    ...
    for (ui = 0; ui < 4; ui++)
    {
        ...
        //*Out++ = (u8_t)(ltemp ^ K)&0xff;
                ...
                *((u8_t *)mic + *micLen + ui) = (u8_t)(ltemp ^ K)&0xff;
#else
		zmw_buf_overlen_writeb(dev, buf, len+ui, (u8_t)(ltemp ^ K)&0xff);
#endif //MAC_OS_USED_FLAG
        ltemp >>= 8;
    }
