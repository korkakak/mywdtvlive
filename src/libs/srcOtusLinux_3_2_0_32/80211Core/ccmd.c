...
/* For ACU utility , set initial value to avoid WZC get error input*/  
...
void zfChangeSSID(zdev_t* dev)
{
   zmw_get_wlan_dev(dev);

   if (wd->ws.MaxSsidIndex ==1 )   //it means no ACU present or ONLY 1 SSID table in ACU
      	return;
   
   wd->ws.ssidIndex ++;
     if(wd->ws.ssidIndex >wd->ws.MaxSsidIndex)
	 wd->ws.ssidIndex=1;	
   if (wd->ws.ssidIndex == 1)
   {
        zfiWlanSetSSID(dev, wd->ws.ssid1, wd->ws.ssid1Len ); 
   }
   if (wd->ws.ssidIndex == 2)
   {
        zfiWlanSetSSID(dev, wd->ws.ssid2, wd->ws.ssid2Len ); 
   }
   if (wd->ws.ssidIndex == 3)
   {
        zfiWlanSetSSID(dev, wd->ws.ssid3, wd->ws.ssid3Len ); 
   }
   zfGetWrapperSetting(dev);
}

void zfChangeBSSID(zdev_t* dev)
{
   zmw_get_wlan_dev(dev);
   if (wd->ws.bBssidTblExist ==0 )  //no prefer APs table in ACU
      	return;
    
   if (wd->ws.ssid1Len)     //ACU contain SSID tables ,didn't use prefer APs
   	return;
   wd->ws.prefBssidIndex ++;
   if(wd->ws.prefBssidIndex >4)
	 wd->ws.prefBssidIndex=1;	
   if (wd->ws.prefBssidIndex == 1)
   {
        zfiWlanSetBssid(dev, wd->ws.prefBssid1); 
   }
   if (wd->ws.prefBssidIndex == 2)
   {
         zfiWlanSetBssid(dev, wd->ws.prefBssid2);
   }
   if (wd->ws.prefBssidIndex == 3)
   {
        zfiWlanSetBssid(dev, wd->ws.prefBssid3);
   }
   if (wd->ws.prefBssidIndex == 4)
   {
         zfiWlanSetBssid(dev, wd->ws.prefBssid4);
   }
   zfGetWrapperSetting(dev);
}
...
u16_t zfWlanEnable(zdev_t* dev)
{
    ...
    if ( wd->sta.EnableHT )
    {
        ...
        {
            if ( (wd->frequency > 3000) && (wd->wlanMode == ZM_MODE_IBSS) && (!wd->BandWidth40) )
...
u16_t zfiWlanDisable(zdev_t* dev, u8_t ResetKeyCache)
{
    ...
    if (wd->sta.SWEncryptEnable != 0)
    {
        ...
        zfZeroMemory((u8_t *) wd->sta.wepKey, 4*32);
    }
...
void zfiWlanSetMonitorEnable(zdev_t* dev, u8_t monitorEnable)
{
    zmw_get_wlan_dev(dev);
    wd->monitorEnable = monitorEnable;
}

void zfiWlanSetNewAPSlectiAlogrithm(zdev_t* dev, u8_t NewAPSelAlog)
{
    zmw_get_wlan_dev(dev);
    wd->NewAPSelAlog = NewAPSelAlog;
}

u8_t* zfiGetAsocReqFrameBody(zdev_t* dev, u16_t* pAsocReqFrameBodySize)
{
    zmw_get_wlan_dev(dev);

    *pAsocReqFrameBodySize = wd->sta.asocReqFrameBodySize;

    return &(wd->sta.asocReqFrameBody[0]);
}

u8_t* zfiGetAsocRspFrameBody(zdev_t* dev, u16_t* pAsocRspFrameBodySize)
{
    zmw_get_wlan_dev(dev);

    *pAsocRspFrameBodySize = wd->sta.asocRspFrameBodySize;

    return &(wd->sta.asocRspFrameBody[0]);
}

u8_t* zfiGetBeaconFrameBody(zdev_t* dev, u16_t* pBeaconFrameBodySize)
{
    zmw_get_wlan_dev(dev);

    *pBeaconFrameBodySize = wd->sta.beaconFrameBodySize;

    return &(wd->sta.beaconFrameBody[0]);
}