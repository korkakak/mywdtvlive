...
void zfLedCtrl_PassiveMode(zdev_t* dev)
{
    u16_t index;
    u8_t ton, toff;
    u32_t tmp;
    ...
    for (index=0; index<ZM_MAX_LED_NUMBER; index++)
    {
        if( wd->ledStruct.LEDState[index] == 1 )//Check Passives state
        {
            ton = ((wd->ledStruct.ledPassiveMode[index] & 0x0f00) >> 8);
            toff = ((wd->ledStruct.ledPassiveMode[index] & 0xf000) >> 12);
            ...
            if ((ton + toff) != 0)
            {
                ...
                if (tmp < ton)
                {
                    zfHpLedCtrl(dev, index, 1);
                }
                else
                {
                    zfHpLedCtrl(dev, index, 0);
                }
            }
            else
            {
...
void zfLedCtrlType1(zdev_t* dev)
{
    ...
    //control 2 Led, GPIO 0 and GPIO 1 
    for (i=0; i<ZM_MAX_LED_NUMBER; i++)
    {
        if( wd->ledStruct.LEDState[i] == 0 )//Check Active state
        {
...
void zfLedCtrlType2(zdev_t* dev)
{
    ...
    if (zfStaIsConnected(dev) != TRUE)
    {
        ...
        else
        {
            ...
            if( wd->ledStruct.LEDState[0] == 0 )//Check Active state
                zfHpLedCtrl(dev, 0, 0);
            if( wd->ledStruct.LEDState[1] == 0 )//Check Active state
                zfHpLedCtrl(dev, 1, 0);                   
        }
    }
    else
    {
        ...
        if ((zfPowerSavingMgrIsSleeping(dev)) && ((wd->ledStruct.ledMode[OperateLED] & 0x8) == 0))
        {
            ...
            if( wd->ledStruct.LEDState[OperateLED] == 0 )//Check Active state
...
void zfLed100msCtrl(zdev_t* dev)
{
    ...
    //check passive mode GPIO
    zfLedCtrl_PassiveMode(dev);
}
