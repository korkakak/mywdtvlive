...
u8_t zfTkipPhase1KeyMix(u32_t iv32, struct zsTkipSeed* pSeed)
{
    ...
    u16_t   tsc1;
    u16_t   i, j;
    u16_t testPattern = 0x1;
    u8_t* test = (u8_t*)&testPattern;
#if 0
    ...
    pSeed->ttak[0] = tsc1;
    pSeed->ttak[1] = tsc0;
    pSeed->ttak[2] = (u16_t) (pSeed->ta[test[1]] + (pSeed->ta[test[0]] <<8));
    pSeed->ttak[3] = (u16_t) (pSeed->ta[2+test[1]] + (pSeed->ta[2+test[0]] <<8));
    pSeed->ttak[4] = (u16_t) (pSeed->ta[4+test[1]] + (pSeed->ta[4+test[0]] <<8));
...
void zfCalTxMic(zdev_t *dev, zbuf_t *buf, u8_t *snap, u16_t snapLen, u16_t offset, u16_t *da, u16_t *sa, u8_t up, u8_t *mic)
{
    ...
    for(i = 0; i < 6; i++)
    {
        zfMicAppendByte(pDa[(i&0xfe)+(((i&0x1)==1)?test[0]:test[1])], pMicKey);
    }

    /* append SA */
    for(i = 0; i < 6; i++)
    {
        zfMicAppendByte(pSa[(i&0xfe)+(((i&0x1)==1)?test[0]:test[1])], pMicKey);
    }
    ...
    for(i = 0; i < snapLen; i++)
    {
        zfMicAppendByte(snap[(i&0xfe)+(((i&0x1)==1)?test[0]:test[1])], pMicKey);
    }
...
void zfTKIPEncrypt(zdev_t *dev, zbuf_t *buf, u8_t *snap, u16_t snapLen, u16_t* mic, u16_t* micLen, u16_t offset, u8_t keyLen, u8_t* key, u32_t* icv)
{
    ...
    zfWEPEncrypt(dev, buf, snap, snapLen, mic, micLen, offset, keyLen, &key[3], iv);
}
