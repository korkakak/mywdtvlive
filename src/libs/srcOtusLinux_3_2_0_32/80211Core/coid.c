...
/* Backup ACU SSID table for future use */
void zfiWlanBackupACUSSID(zdev_t* dev, u8_t* ssid, u8_t ssidLength,u8_t* ssid2, u8_t ssid2Length,u8_t* ssid3, u8_t ssid3Length)
{
    u16_t i;
    zmw_get_wlan_dev(dev);    
    ...
    zmw_declare_for_critical_section();
    wd->ws.ssidIndex = 1;	
    if ( ssidLength  == 0)  //it mean ACU SSID table is Null
    {
        wd->ws.ssidIndex = 1;
	 wd->ws.MaxSsidIndex =1;	
	 wd->ws.ssid1Len =0;
	 return;
    }	 
    zmw_enter_critical_section(dev);	
    ...
    {
         wd->ws.ssid1Len = ssidLength;
         zfMemoryCopy(wd->ws.ssid1, ssid, ssidLength);
	   wd->ws.MaxSsidIndex =1;		 
    }
    if ( (ssid2Length >0) & (ssid2Length <= 32) )
    {
         wd->ws.ssid2Len = ssid2Length;
         zfMemoryCopy(wd->ws.ssid2, ssid2, ssid2Length);
	   wd->ws.MaxSsidIndex =2;		 
    }	
    if ( (ssid3Length >0) & (ssid3Length <= 32) )
    {
         wd->ws.ssid3Len = ssid3Length;
         zfMemoryCopy(wd->ws.ssid3, ssid3, ssid3Length);
	   wd->ws.MaxSsidIndex =3;		 
    }
...
void zfiWlanBackupPrefBssid(zdev_t* dev, u8_t* prefBssid1, u8_t* prefBssid2, u8_t* prefBssid3, u8_t* prefBssid4)
{
    u8_t i,tmp;
    zmw_get_wlan_dev(dev);    
    zmw_declare_for_critical_section();
    zmw_enter_critical_section(dev);	
    if ((prefBssid1 == NULL) || (prefBssid2 == NULL) || (prefBssid3 == NULL) || (prefBssid3 == NULL))
    {
	wd->ws.bBssidTblExist = 0;
       zmw_leave_critical_section(dev);
	return;
    }
	
    wd->ws.prefBssidIndex =1;
    tmp =0;
    for (i=0;i<6;i++)
    {
     tmp |= *(prefBssid1+i);
  ...
  //  DbgPrint(" BSSID Table CheckSum = %02X\n",tmp);	
    if ( tmp )
    {
    zfMemoryCopy(wd->ws.prefBssid1, prefBssid1, 6);
    zfMemoryCopy(wd->ws.prefBssid2, prefBssid2, 6);
    zfMemoryCopy(wd->ws.prefBssid3, prefBssid3, 6);
    zfMemoryCopy(wd->ws.prefBssid4, prefBssid4, 6);
    wd->ws.bBssidTblExist = 1;	
    } else
    	{
    wd->ws.bBssidTblExist = 0;	
    	}
...
//                              wd->sta.encryMode,
            ...
            for (i=0; i < 4; i++)
                zfHpSetDefaultKey(dev, i, wd->sta.encryMode, 
                                 (u32_t*) keyInfo.key, (u32_t*) &keyInfo.key[16]);
#endif 			
            wd->sta.keyId = 4;
#endif  
        ...
        {   /* legacy WEP */
...

            {
                ...
                zm_debug_msg0("key = ");
                for(ii = 0; ii < keyInfo.keyLength; ii++)
                {
                    zm_debug_msg2("", keyInfo.key[ii]);
                }
            }
...
u8_t zfiWlanQuerywmeInfo(zdev_t* dev)
{
    zmw_get_wlan_dev(dev);    

    return wd->ws.staWmeQosInfo;
}
...
const unsigned char* zfiWlanQueryCountryIsoName(zdev_t* dev)
{
    zmw_get_wlan_dev(dev);
    
    return (const unsigned char*)wd->ws.countryIsoName;
}
...
void zfiWlanSetIBSSHTAndAggrEnable(zdev_t* dev, u8_t htAdhoc, u8_t htAdhocHT40, u8_t htAdhocAggr)
{
    zmw_get_wlan_dev(dev);

    wd->sta.adhocEnableHT = htAdhoc;
    wd->sta.adhoc5GHzEnableHT40 = htAdhocHT40;
    wd->sta.adhocEnableAggregation = htAdhocAggr;
}

void zfiWlanSetLedControl(zdev_t*dev, u32_t support, u16_t LedOnTime, u16_t LedOffTime)
{
    u8_t ton, toff, tmp;
    u16_t i;
    ...
    //support mode
    //bit 31-16: Set control GPIO value.
    //0x0: Set GPIO 0
    //0x1: Set GPIO 1
    //Others: Reserved.
    //bit 15-4: Reserved.
    //bit 3: Set Application Control State.
    //0: Back to Active state -back to driver control.
    //1: Go to Passives state - go to application control.
    //bit 2-0: Reserved.
    
    //LedOnTime
    //bit 0-16: LED on time in ms (driver will divide it by 100ms, less than 100ms will be treated as 100ms).
    //0xFFFF: is solid on.
    //0:         is solid off.

    //LedOffTime
    //bit 0-16: LED off time in ms (driver will divide it by 100ms, less than 100ms will be treated as 100ms).

    //WPS Led Mode for LED in application control state
    //bit 15-12 : Toff counter
    //bit 11-8 : Ton counter
    //bit 7-0 : Reserved.

    i = (u16_t)( ((support & 0xffff0000)>>16) & 0xffff);
    zm_debug_msg1("Set index = ",i);
    
    if( i > (ZM_MAX_LED_NUMBER-1))
        return;
    
    if(support & 0x08)//Go to Passives state
    {
        wd->ledStruct.LEDState[i]  = 1;

        //Check LED behavior
        if(LedOnTime == 0xFFFF)
        {
            wd->ledStruct.ledPassiveMode[i] = 0x100;// Led on solid
        }
        else if(LedOnTime == 0x0)
        {
            wd->ledStruct.ledPassiveMode[i] = 0x1000;// Led Off solid
        }
        else
        {
            wd->ledStruct.ledPassiveMode[i] = 0x0; 
            
            ton = LedOnTime/100; //100ms/unit
            toff = LedOffTime/100; //100ms/unit
            ...
            wd->ledStruct.ledPassiveMode[i] |= (((toff & 0x0f)<<12)|((ton & 0x0f)<<8));//Led blinking
        }
    }
    else//Back to Active state
    {
        wd->ledStruct.LEDState[i] = 0;
        wd->ledStruct.ledPassiveMode[i] = 0x0;
    }   
    
    zm_debug_msg1("wd->ledStruct.LEDState = ",wd->ledStruct.LEDState[i]);
    zm_debug_msg1("wd->ledStruct.ledPassiveMode = ",wd->ledStruct.ledPassiveMode[i]);
}

void zfiWlanQueryLedControl(zdev_t* dev, u32_t* support, u16_t* Ton, u16_t* Toff)
{
    u8_t ton, toff, tmp0, tmp1;
    u16_t index;
    
    zmw_get_wlan_dev(dev);    

    //[DWORD] SupportMode
    //bit 31-16: Set query GPIO value .
    //0x0: Query GPIO 0
    //0x1: Query GPIO 1
    //Others: Reserved.
    //bit 15-3: Reserved.
    //bit 2: Current State.
    //0: Active State - Driver control.
    //1: Passive State - Application control.
    //bit 1: Blinking Supported.
    //bit 0: LED for WPS Supported.

    //[WORD] LedOnTime
    //bit 0-16: LED on time in ms (will be multiple of 100ms).
    //0xFFFF: is solid on.
    //0:           is solid off.

    //[WORD] LedOffTime
    //bit 0-16: LED off time in ms (will be multiple of 100ms). 
    
    index =  (u16_t)(((*support & 0xffff0000)>>16) & 0xffff);
    zm_debug_msg1("Query index =",index);

    if( index > (ZM_MAX_LED_NUMBER-1))
        return;
    
    *support = 0x03;// Default set bit0,bit1
    *support |= (wd->ledStruct.LEDState[index] << 2) & 0x04;
    ...
    if(wd->ledStruct.LEDState[index] == 1 )
    {
        toff = (wd->ledStruct.ledPassiveMode[index] & 0xf000)>>12; //100ms/unit
        ton = (wd->ledStruct.ledPassiveMode[index] & 0x0f00)>>8; //100ms/unit
    ...
    zm_debug_msg1("support = ",*support);
    zm_debug_msg1("Ton = ",*Ton);
    zm_debug_msg1("Toff = ",*Toff);
}

void zfiWlanSetAdditionalAssocIE(zdev_t* dev, u32_t additionalAssocIESize, u8_t* additionalAssocIE)
{
    zmw_get_wlan_dev(dev);
	
    if ( additionalAssocIESize ) {
        wd->sta.additionalAssocIESize = additionalAssocIESize;
        zfMemoryCopy(wd->sta.additionalAssocIE, additionalAssocIE, (u16_t)additionalAssocIESize);
    } else
    	wd->sta.additionalAssocIESize = 0;
}