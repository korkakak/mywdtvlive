...
u8_t zcUpToAc[8] = {0, 1, 1, 0, 2, 2, 3, 3}; //WMM default
...
u16_t zfTxSendEth(zdev_t* dev, zbuf_t* buf, u16_t port, u16_t bufType, u16_t flag)
{
   ...
   if ( (up == 7) && ( wd->sta.acmFlag & 0x8))
   	up =5;
   if ( (up == 5) && ( wd->sta.acmFlag & 0x4)) 
       up = 0; 
    if ( (up == 0) && ( wd->sta.acmFlag & 0x1)) 
       up = 1;
    //EOSP bit
    if (newFlag & 0x100)
    {
...
/* ----------------------------------------------------------------------------------------------- */
    ...
    headerLen = zfTxGenWlanHeader(dev, frag.buf[0], header, frag.seq[0],
                                  ...
                                  up, mic, &micLen, snap, snapLen, NULL);	
    ...
    { 
        ...
        {         
            ...
            headerLen = zfTxGenWlanHeader(dev, frag.buf[i], header, frag.seq[i],
                                    frag.flag[i], snapLen+micLen, removeLen, port, da, sa, up, mic, &micLen,
                                    snap, snapLen, NULL);   
...
void zfCoreRecv(zdev_t* dev, zbuf_t* buf, struct zsAdditionInfo* addInfo)
{
    ...
    if ((ret = zfWlanRxValidate(dev, buf)) != ZM_SUCCESS)
    {
        ...
        ret = zfRecoveryFrame(dev, buf);
        ret1=zfWlanRxValidate(dev, buf);
	    if (ret1 != ZM_SUCCESS)
	    	{
        //monitor Mode <Filter MAC not match>
        if ( wd->monitorEnable == 0 )
        {
            goto zlError;
        }
	     }	
    ...
    if ((ret = zfWlanRxFilter(dev, buf)) != ZM_SUCCESS)
    {
        ...
        //monitor Mode <Filter duplicate frame>
        if ( wd->monitorEnable == 0 )
        {
            goto zlError;
        }
    ...
    if ((addInfo->Tail.Data.ErrorIndication & 0x3f) != 0)
    {
        ...
        {
            //monitor Mode <Filter CRC error and Address not match>
            if ( wd->monitorEnable == 0 )
            {
                goto zlError;
            }
...
{
    u8_t   packetType, keyType, code, identifier, type, flags, Op_Code, messageType; 
    u16_t  packetLen, keyInfo, keyLen, keyDataLen, length;
    u32_t  replayCounterH, replayCounterL, vendorId, VendorType;

    zm_debug_msg0("Receive EAPOL frame");
    ...
    { // EAP-Packet
        ...
        {    
            ...
            {    
                ...
                /* EAP-Packet Op Code */
                Op_Code = zmw_rx_buf_readb(dev, buf, offset+16); // 0x01 : WSC_Start
                                                                 // 0x02 : WSC_ACK
                                                                 // 0x03 : WSC_NACK
                                                                 // 0x04 : WSC_Msg
                                                                 // 0x05 : WSC_Done
                                                                 // 0x06 : WSC_FRAG_ACK
                /* EAP-Packet Flags */
                flags = zmw_rx_buf_readb(dev, buf, offset+17);
                ...
                if ( (Op_Code == 0x02) || (Op_Code == 0x03) ||
                	 (Op_Code == 0x04) || (Op_Code == 0x05) )
                {
                    /* EAP-Packet Message Type */
                    messageType = zmw_rx_buf_readb(dev, buf, offset+27); // 0x04 : M1
                                                                         // 0x05 : M2
                                                                         // 0x07 : M3
                                                                         // 0x08 : M4
                                                                         // 0x09 : M5
                                                                         ...
                                                                         // 0x0b : M7
                                                                         // 0x0c : M8
                    zm_debug_msg1("EAP-Packet Message Type = ", messageType);
                }
            }
        }
        else if( code == 2 )
...
{
    u8_t   packetType, keyType, code, identifier, type, Op_Code, flags, messageType; 
    u16_t  packetLen, keyInfo, keyLen, keyDataLen, length;
    u32_t  replayCounterH, replayCounterL, vendorId, VendorType;
...
//    DbgPrint("\n");
    zm_debug_msg0("Send EAPOL frame");
    zm_debug_msg1("EAPOL Packet size = ", zfwBufGetSize(dev, buf));
    ...
    { // EAP-Packet
        ...
        {    
            ...
            {    
                ...
                /* EAP-Packet Op Code */
                Op_Code = zmw_tx_buf_readb(dev, buf, offset+16); // 0x01 : WSC_Start
                                                                 // 0x02 : WSC_ACK
                                                                 // 0x03 : WSC_NACK
                                                                 // 0x04 : WSC_Msg
                                                                 // 0x05 : WSC_Done
                                                                 // 0x06 : WSC_FRAG_ACK
                /* EAP-Packet Flags */
                flags = zmw_tx_buf_readb(dev, buf, offset+17);
                ...
                {
                    /* EAP-Packet Message Type */
                    messageType = zmw_tx_buf_readb(dev, buf, offset+27); // 0x04 : M1
                                                                         // 0x05 : M2
                                                                         // 0x07 : M3
                                                                         // 0x08 : M4
                                                                         // 0x09 : M5
    ...
    if (frameType == ZM_WLAN_DATA_FRAME)
    {
             ...
             (wd->wlanMode == ZM_MODE_IBSS) )
        {
                                           ...
                                           24+offset, 6) )
            {
                ...
                {
                    zfShowRxEAPOL(dev, buf, 32+offset);
                }
            }
            else
            {
...
u16_t zfWlanRxValidate(zdev_t* dev, zbuf_t* buf)
{
    ...
    else if ( wd->wlanMode != ZM_MODE_PSEUDO )
    {
     ...
     if ((zmw_rx_buf_readh(dev, buf, 24) & 0x80) == 0) {  //not AMSDU
         if ((frameLen > 1550) && (wd->sta.encryMode == ZM_AES)) {   
             return ZM_ERR_MAX_RX_FRAME_LENGTH;
         }
         if ((frameLen > 1534) && (wd->sta.encryMode == ZM_NO_WEP)) {
             return ZM_ERR_MAX_RX_FRAME_LENGTH;
         }
     }
