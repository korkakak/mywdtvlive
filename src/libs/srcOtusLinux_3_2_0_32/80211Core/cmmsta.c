...
//
static const STA_MAPPING StaHysTable[] = {
    {35,    7},
    {25,    5},
    {15,    3},
    {10,    2},
...
// 11a table
//
static const STA_MAPPING StaUtility_11a[] = {
...
// 11b table
//
static const STA_MAPPING StaUtility_11b[] = {
...
// 
static const STA_MAPPING StaUtility_11g[] = {
...
// 
static const STA_MAPPING StaUtility_11na[] = {
    { 100,     40600},      // Weak increasing benefit
    {  21,     30500},      //  54 Mbps rate
    {  20,     28500},      //  48 Mbps rate
    {  15,     23700},      //  36 Mbps rate
    {  11,     17700},      //  24 Mbps rate
...
// 
static const STA_MAPPING StaUtility_11ng[] = {
    { 100,     38000},      // Weak increasing benefit
    {  21,     27450},      //  54 Mbps rate
    {  20,     25650},      //  48 Mbps rate
    {  15,     21330},      //  36 Mbps rate
    {  11,     15930},      //  24 Mbps rate
...
int zfStaSetOppositeInfoFromBSSInfo(zdev_t* dev, struct zsBssInfo* pBssInfo)
{
    ...
    // Connect to non-HT Adhoc network
    ...
    if (wd->sta.EnableHT == 1)
    {
	 ...
	 wd->sta.wmeParameterSetCount =-1;	
    }
    if ((wd->sta.wmeEnabled & ZM_STA_WME_ENABLE_BIT) != 0) //WME enabled
    {
        /* Beacon may contain WME parameter element */
        if ( pBssInfo->wmeSupport )
        {
    ...
    if ( wd->sta.adhoc5GHzEnableHT40 || ( wd->frequency < 3000 ) )
    {
        if ( pBssInfo->enableHT40 && 
            ( pBssInfo->extChOffset == 1 || pBssInfo->extChOffset == 3 ) )
        {
            wd->BandWidth40 = pBssInfo->enableHT40;
            wd->ExtOffset = pBssInfo->extChOffset;
        }
        if ( wd->sta.EnableHT && wd->BandWidth40 && wd->ExtOffset )
        {
    ...
    // Legacy (UB82) connect to HT (CB72) need to keep legacy mode !
    if ( (wd->wlanMode == ZM_MODE_IBSS) && (!wd->sta.EnableHT) )
    {
        bAdhocNotSupportHT = 1;
    }
...
void zfAdhocStoreBeaconIe(zdev_t* dev, zbuf_t* buf)
{
    u16_t offset;
    u32_t i;
    u16_t length;
	...
	wd->sta.ie.HtCap.HtCapInfo = zmw_le16_to_cpu(wd->sta.ie.HtCap.HtCapInfo);
...
int zfStaSetOppositeInfoFromRxBuf(zdev_t* dev, zbuf_t* buf)
{
    ...
    u8_t   bAdhocJointerNotSupportHT = 0;
	...
	mac[0] = zmw_cpu_to_le16(sa[0]);
	mac[1] = zmw_cpu_to_le16(sa[1]);
	mac[2] = zmw_cpu_to_le16(sa[2]);
	
    zmw_enter_critical_section(dev);

    res = zfStaFindFreeOpposite(dev, mac, &i);
    //res = zfStaFindFreeOpposite(dev, sa, &i);
    if ( res != 0 )
    {
        goto zlReturn;
    ...
    if ( wd->wlanMode == ZM_MODE_IBSS )
    {
        ...
        if ((wd->sta.wmeEnabled & ZM_STA_WME_ENABLE_BIT) != 0) //WME enabled
        {
            /* Creater determine joiner's beacon contain QoS Inof */
        ...
        // Legacy (UB82) connect to HT (UB82) need to use legacy rate table !
        if ( (offset = zfFindElement(dev, buf, ZM_WLAN_EID_HT_CAPABILITY)) == 0xffff )
        {
            bAdhocJointerNotSupportHT = 1;
        }
    }
    else if (wd->wlanMode == ZM_MODE_INFRASTRUCTURE)
    {
        /* Check whether AP supports one spatial stream only */
        if (wd->sta.EnableHT == 1)
        {
            if ((offset = zfFindElement(dev, buf, ZM_WLAN_PREN2_EID_HTCAPABILITY)) != 0xffff)
            {
                if (zmw_rx_buf_readb(dev, buf, offset+6) == 0)
                {
                    oneTxStreamCap = 1;
                }
            }
    ...
    else
    {
        ...
        if (wd->sta.currentFrequency < 3000)
        {
            ...
            else
            {
                ...
                /* If found OFDM rate in support rate eleminet => 11g */
                if ((offset = zfFindElement(dev, buf, ZM_WLAN_EID_SUPPORT_RATE)) != 0xffff)
                {
                    u16_t len = zmw_rx_buf_readb(dev, buf, offset+1);
                    u16_t j;
                    
                    for (j=0; j<len; j++)
                    {
                        if ((zmw_rx_buf_readb(dev, buf, offset+2+j)&0x7f) == 0x0c)
                        {
                            ofdmSupport = 1;
                            break;
                        }
...
void zfStaReconnect(zdev_t* dev)
{
    ...
    zfFlushVtxq(dev);
    zfChangeSSID(dev);   //if ACU contain SSID table
    zfChangeBSSID(dev); //if ACU contain prefer AP BSSID	
    zfWlanEnable(dev);
...
void zfStaSendBeacon(zdev_t* dev)
{
    ...
    if ( (wd->wlanMode == ZM_MODE_IBSS) && (wd->sta.EnableHT) )
    {
        /* AtherosN HT Capabilities Info */
        offset = zfMmAddAtherosNHTCapability(dev, buf, offset);
        /* AtherosN Extended HT Capabilities Info */
        offset = zfMmAddAtherosNExtendedHTCapability(dev, buf, offset);
...
zlUpdateRssi:
	...
	{
         pBssInfo->sortValue = zfComputeBssInfoWeightValue_new(dev, 
                                               (pBssInfo->supportedRates[6] + pBssInfo->extSupportedRates[0]), 
                                               pBssInfo->EnableHT,
                                               pBssInfo->channel, 
                                               pBssInfo->signalStrength);	
    ...
    if (zfStaIsConnected(dev))
    {
        ...
        else if ( wd->wlanMode == ZM_MODE_IBSS )
        {
            if ( zfRxBufferEqualToStr(dev, buf, bssid, ZM_WLAN_HEADER_A3_OFFSET, 6) )
            {
                ...
                result = zfStaSetOppositeInfoFromRxBuf(dev, buf);
                if (result == 0)
                {
  	...
  	if ((apMacAddr[0] == wd->sta.bssid[0]) && (apMacAddr[1] == wd->sta.bssid[1]) && (apMacAddr[2] == wd->sta.bssid[2]))			
        {
            if (zfwBufGetSize(dev, buf) >= 24+2) //not a malformed frame
            {
                if ( zfStaIsConnected(dev) )
                {                    
                    zfStaResetPKInstalledAndConnectionReasso(dev, (u8_t *)wd->sta.bssid);                 
...
void zfIBSSSetupBssDesc(zdev_t *dev)
{
    ...
    /* Indicate HT Capability to OS */
    if ( wd->sta.EnableHT )
    {
        // (HT 5GHz) CB72 only support HT20
        if ( (wd->frequency > 3000) && (wd->wlanMode == ZM_MODE_IBSS) && (!wd->sta.adhoc5GHzEnableHT40) )
        {
            /* 5GHz HT Capabilities Info */
            wd->sta.HTCap.Byte[2] = 0x0C;
            wd->sta.HTCap.Byte[3] = 0x00;
            wd->sta.HTCap.Byte[4] = 0x1B;

            /* 5GHz Extended HT Capabilities Info */
            wd->sta.ExtHTCap.Byte[2] = 0x30;
            wd->sta.ExtHTCap.Byte[3] = 0x00;
            wd->sta.ExtHTCap.Byte[4] = 0x01;
        }
	
        /* AtherosN HT Capabilities Info */
        {
            pBssInfo->frameBody[offset++] = ZM_WLAN_EID_HT_CAPABILITY;
            pBssInfo->frameBody[offset++] = wd->sta.HTCap.Data.Length;
            for (j = 0; j < 26; j++)
            {
                pBssInfo->frameBody[offset++] = wd->sta.HTCap.Byte[j+2];
            }
        }

        /* AtherosN Extended HT Capabilities Info */
        {
            pBssInfo->frameBody[offset++] = ZM_WLAN_EID_EXTENDED_HT_CAPABILITY;
            pBssInfo->frameBody[offset++] = wd->sta.ExtHTCap.Data.Length;
            for (j = 0; j < 22; j++)
            {
                pBssInfo->frameBody[offset++] = wd->sta.ExtHTCap.Byte[j+2];
            }
        }

        /* Indicate WME Info to OS */
        /* WME Info */
        if ( wd->sta.wmeEnabled )
        {
            pBssInfo->frameBody[offset++] = ZM_WLAN_EID_WIFI_IE;
            pBssInfo->frameBody[offset++] = 7;

            pBssInfo->frameBody[offset++] = 0x00;
            pBssInfo->frameBody[offset++] = 0x50;
            pBssInfo->frameBody[offset++] = 0xF2;
            pBssInfo->frameBody[offset++] = 0x02;
            pBssInfo->frameBody[offset++] = 0x00;
            pBssInfo->frameBody[offset++] = 0x01;
        ...
        /* HT Capabilities Info */
        {
            ...
            pBssInfo->frameBody[offset++] = ZM_WLAN_EID_WPA_IE ;
            pBssInfo->frameBody[offset++] = wd->sta.HTCap.Data.Length + 4 ;
            for (j = 0; j < 3; j++)
            {
                pBssInfo->frameBody[offset++] = OUI[j] ;
            }

            pBssInfo->frameBody[offset++] = wd->sta.HTCap.Data.ElementID ;
            for (j = 0; j < 26; j++)
            {
                pBssInfo->frameBody[offset++] = wd->sta.HTCap.Byte[j+2] ;
            }
        ...
        /* Extended HT Capabilities Info */
        {
            ...
            pBssInfo->frameBody[offset++] = ZM_WLAN_EID_WPA_IE ;
            pBssInfo->frameBody[offset++] = wd->sta.ExtHTCap.Data.Length + 4 ;
            for (j = 0; j < 3; j++)
            {
                pBssInfo->frameBody[offset++] = OUI[j] ;
            }

            pBssInfo->frameBody[offset++] = wd->sta.ExtHTCap.Data.ElementID ;
            for (j = 0; j < 22; j++)
            {
                pBssInfo->frameBody[offset++] = wd->sta.ExtHTCap.Byte[j+2] ;
            }
...
void zfIbssConnectNetwork(zdev_t* dev)
{
    ...
    if ( !wd->XLinkMode && wd->sta.adhocEnableHT )
    {
        wd->sta.EnableHT = wd->sta.adhocEnableHT;
    }
    ...
    if ( wd->sta.bssList.bssCount )
    {
        ...
        if (pBssInfo != NULL)
        {
            ...
            // Record the frequency of the PSP that created Adhoc network
            wd->PSPFrequency = wd->frequency;            
    ...
    if ( bssNotFound )
    {
        ...
        // Use the PSP frequency to create an Ad hoc network for PSP to re-connect
        if ( wd->XLinkMode && wd->PSPFrequency )
            wd->frequency = wd->PSPFrequency;
        ...
        struct zsBssInfo* candidateBss)
{
    ...
    if (pNowBssInfo != NULL)
    {
        zfwMemoryCopy((u8_t*)candidateBss, (u8_t*)pNowBssInfo, sizeof(struct zsBssInfo));
...
#if 0   //regrade 20090205
u16_t zfRecoveryFrame(zdev_t* dev, zbuf_t* buf)
{
    u16_t len,i,j,cpyindex=0;
    u16_t CmpData[6];
    u8_t  *test;
    struct zsBuffer* pBuf;
  
    zmw_get_wlan_dev(dev);    
    pBuf = (struct zsBuffer*) buf;
    test = (u8_t *)buf;
    len = zfwBufGetSize(dev, buf); 
    i=0;
    if ( wd->wlanMode == ZM_MODE_INFRASTRUCTURE ) {
        zfwMemoryCopy((u8_t *)(CmpData), (u8_t *)(wd->macAddr), 6);
        CmpData[3] = zmw_cpu_to_le16(wd->sta.bssid[0]); 
        CmpData[4] = zmw_cpu_to_le16(wd->sta.bssid[1]); 
        CmpData[5] = zmw_cpu_to_le16(wd->sta.bssid[2]);
        for (i=4; i<len-12; i++)
        {
            for (j=0; j<6; j++) {
                 if (zmw_rx_buf_readh(dev, buf, i+j*2) == CmpData[j])
                     continue;
                 else
                     break;
             }
        ...
        if(cpyindex) {
            if (len > cpyindex) {
                ...
                for (i=0;i<(len-cpyindex);i++)
                 {
                 //zmw_rx_buf_writeb(dev, buf, i,*(test+i)); 
                   zmw_rx_buf_writeb(dev, buf, i, zmw_rx_buf_readb(dev, buf, i+cpyindex));
                 }

                   zfwBufSetSize(dev, buf, len-cpyindex);		
            }	 
        }
    }
    return cpyindex;	
...
u16_t zfRecoveryFrame(zdev_t* dev, zbuf_t* buf)
{
   ...
   struct zsBuffer* pBuf;
    ...
    if ( wd->wlanMode == ZM_MODE_INFRASTRUCTURE )	
    {
	  ...
	  for (i=4;i<len-12;i++)
	  {
           if (zfwMemoryIsEqual( test+i, (u8_t *)CmpData,12))
              {
              cpyindex =i;
	      }
	  }
...
void zfStaIbssMonitoring(zdev_t* dev, u8_t reset)
{
    ...
    for(i=0; i < ZM_MAX_OPPOSITE_COUNT; i++)
    {
        ...
        // If start XLinkKai/MengZone, then the Otus won't indicate the disconnection !
        if ( wd->XLinkMode )
            wd->sta.oppositeInfo[i].aliveCounter = ZM_IBSS_PEER_ALIVE_COUNTER;
    }    
...
u16_t zfComputeBssInfoWeightValue(zdev_t *dev, u8_t isBMode, u8_t isHT, u8_t isHT40, u8_t signalStrength)
{
	...
	u8_t  weightOfN20BelowThr = 25;
	u8_t  weightOfN20UpThr    = 30;
	u8_t  weightOfN40BelowThr = 26;
...
//
// StaMapValue maps an input value to an output value using
...
StaMapValue(
    u16_t   inVal, 
    const    STA_MAPPING    *mappingTable,
    u16_t    numMappingEntries
    )
{
    u16_t    outVal, i, inStep, inPos;
...
u16_t zfComputeBssInfoWeightValue_new(zdev_t *dev, u8_t isBMode, u8_t isHT, u8_t channel, u8_t signalStrength)
{
  u16_t  LinkQuality=0;
  u16_t  WeightValue=0;
  LinkQuality = signalStrength*2;
//  DbgPrint(" Link Quality = %d [%d]\n",LinkQuality,isBMode);
//  DbgPrint(" Channel = %d HT=%d \n",channel,isHT);
//  WeightValue= StaMapValue(signalStrength*2, *PhyTable, 10);
if ( isBMode == 0 )
  return StaMapValue(LinkQuality,StaUtility_11b,ARRAY_LENGTH(StaUtility_11b));	

if(channel >= 14)  //A-BAND
{
   if (isHT ==1 )
   	{
   	WeightValue = StaMapValue(LinkQuality,StaUtility_11na,ARRAY_LENGTH(StaUtility_11na));
   	}
   	else
   	{
   	WeightValue = StaMapValue(LinkQuality,StaUtility_11a,ARRAY_LENGTH(StaUtility_11a));
   	}	
}else
{
if (isHT ==1 )
   	{
   	WeightValue = StaMapValue(LinkQuality,StaUtility_11ng,ARRAY_LENGTH(StaUtility_11ng));
   	}
   	else
   	{
   	WeightValue = StaMapValue(LinkQuality,StaUtility_11g,ARRAY_LENGTH(StaUtility_11g));
   	}	
}
 return WeightValue;
}   
...
void zfStaResetPKInstalledAndConnectionReasso(zdev_t *dev, u8_t *staMacAddr)
{
    u8_t   res, peerIdx;
    zmw_get_wlan_dev(dev);
    ...
    if( res == 0 ) {
        wd->sta.oppositeInfo[peerIdx].pkInstalled = 0;
        wd->sta.connectByReasso = FALSE;
    }
    zmw_leave_critical_section(dev);
}

u16_t zfStaAddAdditionalAssocIE(zdev_t* dev, zbuf_t* buf, u16_t offset)
{
    u16_t i;
	
    zmw_get_wlan_dev(dev);    

    for (i=0; i<wd->sta.additionalAssocIESize; i++) {
        zmw_tx_buf_writeb(dev, buf, offset++, wd->sta.additionalAssocIE[i]);
    }
