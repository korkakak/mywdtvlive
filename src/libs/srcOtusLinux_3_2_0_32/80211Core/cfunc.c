                    ...
                    u16_t dstOffset, u16_t srcOffset, u16_t length)
{
    ...
    pDes = (u8_t *)zmw_buf_get_buffer(dev,	dst);
    pDes += dstOffset;
    pSrc = (u8_t *)zmw_buf_get_buffer(dev, src); 	
    pSrc +=  srcOffset;
    zfMemoryMove(pDes,pSrc,length);
/*
    u16_t i;
    for(i=0; i<length; i++)
    {
 ...
 */   
#else
	zmw_buf_copy(dev, dst, src, dstOffset, srcOffset, length);
#endif
}
