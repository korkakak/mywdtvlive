...
u8_t zfScanMgrScanStart(zdev_t* dev, u8_t scanType)
{
    ...
    zm_msg1_scan(ZM_LV_1, "scanType = ", scanType);
    ...
    {
        zm_msg1_scan(ZM_LV_1, "scan rescheduled", scanType);                
        goto scan_done;
    }
    
    wd->sta.scanMgr.scanReqs[i] = 1;
    zm_msg1_scan(ZM_LV_1, "scan scheduled: ", scanType);       
...
{                       
         ...
         || (wd->sta.scanFrequency == zfChGetLastChannel(dev, &temp)) )
    {
        ...
        zm_msg1_scan(ZM_LV_1, "scan 1 type: ", wd->sta.scanMgr.currScanType);
        zm_msg1_scan(ZM_LV_1, "scan channel count = ", wd->regulationTable.allowChannelCnt);
