...
char *usbdrv_translate_scan(struct net_device *dev, char *current_ev,
        char *end_buf, struct zsBssInfo *list, struct iw_request_info *info)
{
...
#define IEEE80211_ELEMID_RSN 0x30
	...
	if (list->wscIe[1] != 0)
    {
	/* AVM/JBR beacon does not add wscIe ? Some scans only show large WscIE from ProbeResps*/
        static const char wsc_leader[] = "wps_ie=";

	//printk("Got wsc ie: len %u\n", list->wscIe[1]+2);
        memset(&iwe, 0, sizeof(iwe));

        iwe.cmd = IWEVCUSTOM;
	iwe.u.data.length = encode_ie(buf, sizeof(buf),
				      list->wscIe, list->wscIe[1]+2,
            ...
            union iwreq_data *wrq, char *extra)
{
    ...
    WlanMode = zfiWlanQueryWlanMode(dev);
    switch(wrq->mode)
    {
        case IW_MODE_MASTER:
            if (WlanMode == ZM_MODE_AP) return 0;
            WlanMode = ZM_MODE_AP;
            break;
        case IW_MODE_INFRA:
            if (WlanMode == ZM_MODE_INFRASTRUCTURE) return 0;
            WlanMode = ZM_MODE_INFRASTRUCTURE;
            break;
        case IW_MODE_ADHOC:
            if (WlanMode == ZM_MODE_IBSS) return 0;
...
// XXX what about min/max_pmp, min/max_pmt, etc.
    ...
    {
        range->num_channels = 11; // Only 2.4G
        range->num_frequency = 11;
/* XXX need to filter against the regulatory domain &| active set */
    val = 0;
        for (i = 1; i <= 11; i++) // B,G Bands
    {
        range->freq[val].i = i;
        if (i == 14)  
        ...
        struct iw_point *data, char *extra)
{
    ...
    //Avoid Scan in Chariot test  
    if (macp->adapterState == ZM_STATUS_MEDIA_CONNECT)
    {
        macp->ReceivedPktRatePerSecond = zfiWlanQueryReceivedPacket(dev);
        
        if( ( macp->PktRatePerSecond + macp->ReceivedPktRatePerSecond ) > 650 )
        {
            printk(KERN_DEBUG "Block background scan!\n");
            return 0;
        }
    }
            ...
            struct iw_point *data, char *extra)
{
    ...
    {
        /* Scanning */
        if (macp->scanNotReadyCount++ < 20)
        {
            return -EAGAIN;
        }
    }
    macp->scanNotReadyCount = 0;
...
/* Translate to WE format this entry */
        //current_ev = usbdrv_translate_scan(dev, current_ev,
        //    extra + IW_SCAN_MAX_DATA, &pBssList->bssInfo[i]);
        current_ev = usbdrv_translate_scan(dev, current_ev,
            end_buf, &pBssList->bssInfo[i], info);
    ...
    switch (cmd)
    {
        ...
        case ZD_IOCTL_PARAM:
        {
            ...
            if(op == ZD_PARAM_PRIVACY)
            {
                ...
                //    mCap[0] |= CAP_PRIVACY;
                //    macp->cardSetting.EncryOnOff[0] = 1;
                    zfiWlanSetWepStatus(dev, ZM_ENCRYPTION_WEP_ENABLED);
...
#if WIRELESS_EXT >= 18
int usbdrvwext_siwmlme(struct net_device *dev,
        struct iw_request_info *info, struct iw_point *erq, char *data)
{
    struct iw_mlme *wextmlme = (struct iw_mlme *)data;
    struct athr_wlan_param param;
    int ret = 0;

    memset(&param, 0, sizeof(struct athr_wlan_param));
    ...
    param.u.mlme.reason_code = wextmlme->reason_code;
    memcpy(param.sta_addr, wextmlme->addr.sa_data, IEEE80211_ADDR_LEN);
    ret = usbdrv_wpa_ioctl(dev, &param);

    return ret;
}

int usbdrvwext_siwgenie(struct net_device *dev,
        struct iw_request_info *info, struct iw_point *erq, char *data)
{
    struct athr_wlan_param *param;
    int ret;
    size_t blen = ZD_GENERIC_ELEMENT_HDR_LEN + erq->length;

    /* TODO: Check the erq->length */

    if (blen < sizeof(*param))
        blen = sizeof(*param);

    param = (struct athr_wlan_param *) kmalloc(blen, GFP_KERNEL);
    if (param == NULL)
        return -1;

    /* Make the format translation */
    memset(param, 0, blen);
    param->cmd = ZD_CMD_SET_GENERIC_ELEMENT;
    param->u.generic_elem.len = erq->length;
    memcpy(param->u.generic_elem.data, data, erq->length);
...
int usbdrvwext_giwgenie(struct net_device *dev,
        struct iw_request_info *info, struct iw_point *out, char *buf)
{
    return 0;
}

int usbdrvwext_siwauth(struct net_device *dev,
        struct iw_request_info *info, struct iw_param *erq, char *buf)
{
    ...
    case IW_AUTH_WPA_VERSION:
        printk(KERN_WARNING "IW_AUTH_WPA_VERSION\n");
	ver = erq->value;
        ...
        if (ver == IW_AUTH_WPA_VERSION_WPA)
        {
            if ((macp->supIe[21] == 0x50) && 
                ...
                (macp->supIe[23] == 0x2))
            {
                printk(KERN_DEBUG "wd->sta.authMode = ZM_AUTH_MODE_WPAPSK\n");
                zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_WPAPSK);
            }
            else if ((macp->supIe[21] == 0x50) && 
                     (macp->supIe[22] == 0xf2) && 
                     (macp->supIe[23] == 0x1))
            {
                printk(KERN_DEBUG "wd->sta.authMode = ZM_AUTH_MODE_WPA\n");
                zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_WPA);
            }
        }
        else if (ver == IW_AUTH_WPA_VERSION_WPA2)
        {
            if ((macp->supIe[17] == 0xf) && 
                ...
                (macp->supIe[19] == 0x2))
            {
                printk(KERN_DEBUG "wd->sta.authMode = ZM_AUTH_MODE_WPA2PSK\n");
                zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_WPA2PSK);
            }
            else if ((macp->supIe[17] == 0xf) && 
                     (macp->supIe[18] == 0xac) && 
                     (macp->supIe[19] == 0x1))
            {
                 printk(KERN_DEBUG "wd->sta.authMode = ZM_AUTH_MODE_WPA2\n");
                 zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_WPA2);
            }
        }
        else
        {
            printk(KERN_DEBUG "Unsupported IW_AUTH_WPA_VERSION: %d\n", ver);
            rc = -EINVAL;
        }

        break;
    case IW_AUTH_CIPHER_PAIRWISE:
        printk(KERN_WARNING "IW_AUTH_CIPHER_PAIRWISE\n");
    ...
    case IW_AUTH_CIPHER_GROUP:
        printk(KERN_WARNING "IW_AUTH_CIPHER_GROUP\n");
        break;
    case IW_AUTH_KEY_MGMT:
        printk(KERN_WARNING "IW_AUTH_KEY_MGMT\n");
        break;
    case IW_AUTH_TKIP_COUNTERMEASURES:
        printk(KERN_WARNING "IW_AUTH_TKIP_COUNTERMEASURES\n");
        break;
    case IW_AUTH_DROP_UNENCRYPTED:
        printk(KERN_WARNING "IW_AUTH_DROP_UNENCRYPTED\n");
        break;
    case IW_AUTH_80211_AUTH_ALG:
        printk(KERN_WARNING "IW_AUTH_80211_AUTH_ALG\n");
        if (erq->value == IW_AUTH_ALG_SHARED_KEY)
        {
            printk(KERN_WARNING "IW_AUTH_ALG_SHARED_KEY\n");
            zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_SHARED_KEY);
        }
        else if (erq->value == IW_AUTH_ALG_OPEN_SYSTEM)
        {
            printk(KERN_WARNING "IW_AUTH_ALG_OPEN_SYSTEM\n");
            zfiWlanSetAuthenticationMode(dev, ZM_AUTH_MODE_OPEN);
        }
        break;
    case IW_AUTH_WPA_ENABLED:
        printk(KERN_WARNING "IW_AUTH_WPA_ENABLED\n");
        break;
    case IW_AUTH_RX_UNENCRYPTED_EAPOL:
        printk(KERN_WARNING "IW_AUTH_RX_UNENCRYPTED_EAPOL\n");
        break;
    case IW_AUTH_ROAMING_CONTROL:
        printk(KERN_WARNING "IW_AUTH_ROAMING_CONTROL\n");
        break;
    case IW_AUTH_PRIVACY_INVOKED:
        printk(KERN_WARNING "IW_AUTH_PRIVACY_INVOKED: %d\n", erq->value);

        if (erq->value)
        {
			if(zfiWlanQueryWepStatus(dev,1))
			{
					//printk(KERN_WARNING "Do nothing ================n");
			}
			else
			{		
					
					zfiWlanSetAuthenticationMode(dev, 2);
					zfiWlanSetWepStatus(dev, ZM_ENCRYPTION_WEP_ENABLED);
			}
        }
        else
...
int usbdrvwext_giwauth(struct net_device *dev,
        struct iw_request_info *info, struct iw_param *erq, char *buf)
{
    return 0;
}

int usbdrvwext_siwencodeext(struct net_device *dev,
        struct iw_request_info *info, struct iw_point *erq, char *extra)
{
    ...
    case IW_ENCODE_ALG_NONE:
        printk(KERN_ERR "%s: IW_ENCODE_ALG_NONE\n", __FUNCTION__);
        return 0;
    case IW_ENCODE_ALG_WEP:
    {
        ...
        printk(KERN_ERR "%s: IW_ENCODE_ALG_WEP\n", __FUNCTION__);
        ...
        return usbdrvwext_siwencode(dev, &oldinfo, &olderq, ext->key);
    }
        break;
    ...
    if (ext->ext_flags & IW_ENCODE_EXT_GROUP_KEY)
        ...
        memcpy(param->sta_addr, ext->addr.sa_data, ETH_ALEN);

    key_idx = (erq->flags & IW_ENCODE_INDEX) - 1;
    if (key_idx >= 4)
        key_idx = 0;

    strncpy((char *)param->u.crypt.alg, alg_name, ZD_CRYPT_ALG_NAME_LEN);
    param->u.crypt.flags = (ext->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) ? 
                           ZD_FLAG_SET_TX_KEY : 0;
    param->u.crypt.idx = key_idx;

    if (ext->ext_flags & IW_ENCODE_EXT_RX_SEQ_VALID)
        memcpy(param->u.crypt.seq, ext->rx_seq, 8);

    param->u.crypt.key_len = ext->key_len;
    memcpy((u8 *) param->u.crypt.key, ext->key, ext->key_len);
...
int
usbdrvwext_giwencodeext(struct net_device *dev, 
	struct iw_request_info *info, struct iw_point *erq, char *extra)
{
    return 0;
}
