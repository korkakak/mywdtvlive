...
void zfiUsbRecv(zdev_t *dev, zbuf_t *buf)
{
    ...
    if (usbRxRemainLen != 0 )
    {
        ...
        if ( BufPtr != NULL )
        {
#ifndef MAC_OS_USED_FLAG
            zfwMemoryCopy(&(BufPtr[usbRxPktLen]), srcBufPtr, usbRxRemainLen);
#else
			//mbuf_copyback(halPriv->remainBuf, usbRxPktLen, usbRxRemainLen, srcBufPtr, MBUF_DONTWAIT);
			zfRxBufferCopy(dev, remainBufPtr, buf, usbRxPktLen, 0, usbRxRemainLen);
#endif
...
#else
		//zmw_buf_readb(zdev_t* dev, zbuf_t* buf, u16_t offset)
		pktLen = zmw_buf_readb(dev, buf,index) +(zmw_buf_readb(dev, buf,index+1)<<8);
		pktTag = zmw_buf_readb(dev, buf,index+2) +(zmw_buf_readb(dev, buf,index+3)<<8);
#endif
...
#else
					zfRxBufferCopy(dev, newBuf, buf, 0, (chkIdx+ZM_USB_STREAM_MODE_TAG_LEN), halPriv->usbRxTransferLen);
#endif
...
#else
					zfRxBufferCopy(dev, newBuf, buf, 0, (chkIdx+ZM_USB_STREAM_MODE_TAG_LEN), pktLen);
#endif
...
void zfiUsbRegIn(zdev_t* dev, u32_t* rsp, u16_t rspLen)
{
    ...
    if ((type & 0xC0) == 0xC0)
    {
        ...
        else if (type == 0xC1)
        {
            ...
            status = (u16_t)(zmw_le32_to_cpu(rsp[3]) >> 16);            
            ////6789
            rsp[8] = zmw_le32_to_cpu(rsp[8]) >> 2 | (zmw_le32_to_cpu(rsp[9]) & 0x1) << 6;
...
/* 
 * endian : please handle it by zfwUsbSubmitControl(, FIRMWARE_DOWNLOAD,) !
 */
