...
void zfIdlRsp(zdev_t* dev, u32_t* rsp, u16_t rspLen)
{
    ...
    /* endian issue */
    for (i = 0; i < (rspLen>>2); i++)
    {
        rsp[i] = zmw_le32_to_cpu(rsp[i]);
    }
...
/*      Bit2: Noise calibration finish, but NF value unexcepted => 1 */        
        ...
        {            
            ...
            zm_msg2_scan(ZM_LV_1, " return complete, ret = ", rsp[1]);
...
/*#ifdef ZM_FB50        
        ...
        {
            zm_msg0_mm(ZM_LV_0, "1 Tx stream");
            hpPriv->halCapability |= ZM_HP_CAP_11N_ONE_TX_STREAM;
        }
        else
        {
            zm_msg0_mm(ZM_LV_0, "2 Tx stream");
        }
        
        if (((rsp[2] >> 16)&0xff) == 0x1) //Rx mask == 0x1
        {
            zm_msg0_mm(ZM_LV_0, "1 Rx stream");
            hpPriv->halCapability |= ZM_HP_CAP_11N_ONE_RX_STREAM;
        }
        else
        {
            zm_msg0_mm(ZM_LV_0, "2 Rx stream");
        }
    ...
    else if (src == ZM_EEPROM_READ)
    {
        ...
        for (i=0; i<ZM_HAL_MAX_EEPROM_PRQ; i++)
        {
            ...
            {
                /* 
                 * rsp[] translate endian issue above, so make sure btye order in little endian. 
                 */
                hpPriv->eepromImage[hpPriv->eepromImageIndex++] = zmw_cpu_to_le32(rsp[i+1]);
            }
        }
        ...
        if (hpPriv->eepromImageIndex == (ZM_HAL_MAX_EEPROM_REQ*ZM_HAL_MAX_EEPROM_PRQ))
        {
            ...
            hpPriv->halCapability |= ZM_HP_CAP_HWBTN_EXIST_VALID;
            /*add for check Hardware push button*/
            if(zfGetHwBtnExist(dev))
                hpPriv->halCapability |= ZM_HP_CAP_HWBTN_EXIST;
...
u32_t zfiResetToBootCode(zdev_t* dev)
{
    u32_t cmd[1];
    u16_t ret;
	zmw_get_wlan_dev(dev);
	...
	/* len[0] : type[0x94] : seq[?] */
   cmd[0] = 0 | (ZM_CMD_RESET_TO_BOOT<< 8);    
    ret = zfIssueCmd(dev, cmd, 4, ZM_CMD_RESET_TO_BOOT, 0);
