...
extern u32_t zcFwImage[];
extern u32_t zcFwImageSize;
extern u32_t zcDKFwImage[];
extern u32_t zcDKFwImageSize;
extern u32_t zcFwImageSPI[];
extern u32_t zcFwImageSPISize;
...
#ifdef ZM_OTUS_LINUX_PHASE_2
extern u16_t zfLnxNeedDownloadFw(zdev_t* dev);
#endif
...
void zfInitPhy(zdev_t* dev,  u32_t frequency, u8_t bw40)
{
    ...
    if ( frequency > ZM_CH_G_14 ) 
    {
        ...
        {
            modesIndex = 2;
            zm_msg0_scan(ZM_LV_1, "init ar5416Modes in 2: A-20/40");
        }
        else
        {
            modesIndex = 1;
            zm_msg0_scan(ZM_LV_1, "init ar5416Modes in 1: A-20");
        }
    }
    else
    {
        ...
        {
            modesIndex = 3;
            zm_msg0_scan(ZM_LV_1, "init ar5416Modes in 3: G-20/40");
        }
        else
        {
            modesIndex = 4;
            zm_msg0_scan(ZM_LV_1, "init ar5416Modes in 4: G-20");            
        }
    }
...
//        }
    ...
    //Ant control common
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x144*2/4]);
    eepromBoardData[0][1] = tmp;
    eepromBoardData[0][2] = tmp;
    //Ant control chain 0
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x140*2/4]);
    eepromBoardData[1][1] = tmp;
    eepromBoardData[1][2] = tmp;
    //Ant control chain 2
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x142*2/4]);
    eepromBoardData[2][1] = tmp;
    eepromBoardData[2][2] = tmp;
    //SwSettle
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x146*2/4]);
    tmp = (tmp >> 16) & 0x7f;
    eepromBoardData[3][1] &= (~((u32_t)0x3f80));
    eepromBoardData[3][1] |= (tmp << 7);
#if 0
    //swSettleHt40
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x158*2/4]);
    tmp = (tmp) & 0x7f;
    eepromBoardData[3][2] &= (~((u32_t)0x3f80));
    eepromBoardData[3][2] |= (tmp << 7);
#endif
    //adcDesired, pdaDesired
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x148*2/4]);
    tmp = (tmp >> 24);
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14a*2/4]);
    tmp1 = tmp1 & 0xff;
    tmp = tmp + (tmp1<<8);
    ...
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14a*2/4]);
    tmp = (tmp >> 24) & 0xff;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14c*2/4]);
    tmp1 = (tmp1 >> 8) & 0xff;
    tmp = (tmp<<24) + (tmp<<16) + (tmp1<<8) + tmp1;
    eepromBoardData[5][1] = tmp;
    eepromBoardData[5][2] = tmp;
    //TxEnaToRxOm
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14c*2/4]) & 0xff;
    eepromBoardData[6][1] &= (~((u32_t)0xff0000));
    eepromBoardData[6][1] |= (tmp<<16);
    eepromBoardData[6][2] &= (~((u32_t)0xff0000));
    eepromBoardData[6][2] |= (tmp<<16);
    //Thresh62
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14c*2/4]);
    tmp = (tmp >> 16) & 0x7f;
    ...
    //TxRxAtten chain_2
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x148*2/4]) & 0x3f;
    ...
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x14e*2/4]);
    tmp = (tmp >> 24) & 0x3f;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x150*2/4]);
    tmp1 = (tmp1 >> 8) & 0x1f;
    tmp  = (tmp<<5) + tmp1;
    ...
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x150*2/4]);
    tmp = tmp & 0x3f;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x150*2/4]);
    tmp1 = (tmp1 >> 16) & 0x1f;
    tmp  = (tmp<<5) + tmp1;
    ...
    //bsw_Margin chain_0
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x156*2/4]);
    tmp = (tmp >> 16) & 0xf;
    ...
    //Ant control common
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x170*2/4]);
    tmp = tmp >> 24;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x172*2/4]);
    tmp = tmp + (tmp1 << 8);
    eepromBoardData[0][3] = tmp;
    eepromBoardData[0][4] = tmp;
    //Ant control chain 0
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x16c*2/4]);
    tmp = tmp >> 24;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x16e*2/4]);
    tmp = tmp + (tmp1 << 8);
    eepromBoardData[1][3] = tmp;
    eepromBoardData[1][4] = tmp;
    //Ant control chain 2
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x16e*2/4]);
    tmp = tmp >> 24;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x170*2/4]);
    tmp = tmp + (tmp1 << 8);
    eepromBoardData[2][3] = tmp;
    eepromBoardData[2][4] = tmp;
    //SwSettle
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x174*2/4]);
    tmp = (tmp >> 8) & 0x7f;
    eepromBoardData[3][4] &= (~((u32_t)0x3f80));
    eepromBoardData[3][4] |= (tmp << 7);
#if 0
    //swSettleHt40
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x184*2/4]);
    tmp = (tmp >> 24) & 0x7f;
    eepromBoardData[3][3] &= (~((u32_t)0x3f80));
    eepromBoardData[3][3] |= (tmp << 7);
#endif
    //adcDesired, pdaDesired
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x176*2/4]);
    tmp = (tmp >> 16) & 0xff;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x176*2/4]);
    tmp1 = tmp1 >> 24;
    tmp = tmp + (tmp1<<8);
    ...
    //TxEndToXpaOff, TxFrameToXpaOn
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x178*2/4]);
    tmp = (tmp >> 16) & 0xff;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x17a*2/4]);
    tmp1 = tmp1 & 0xff;
    tmp = (tmp << 24) + (tmp << 16) + (tmp1 << 8) + tmp1;
    ...
    //Thresh62
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x17a*2/4]);
    tmp = (tmp >> 8) & 0x7f;
    ...
    //iqCall chain_0, iqCallQ chain_0
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x17c*2/4]);
    tmp = (tmp >> 16) & 0x3f;
    tmp1 = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x17e*2/4]);
    tmp1 = (tmp1) & 0x1f;
    tmp  = (tmp<<5) + tmp1;
...
//    DbgPrint("xpd=0x%x, pd=0x%x\n", tmp, zcXpdToPd[tmp]);
    ...
    //bsw_Atten chain_0
    tmp = zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x182*2/4]);
    tmp = (tmp>>24) & 0x1f;
    ...
    if ((zmw_le32_to_cpu(hpPriv->eepromImage[0x100+0x110*2/4])&0xff) == 0x80) //FEM TYPE
    {
...
//        zfiCheckRifs(dev);
	...
	if ((hpPriv->halCapability & ZM_HP_CAP_11N_ONE_RX_STREAM) != 0)
    {
        //zfDelayWriteInternalReg(dev, 0x1c59a4, 1);
        reg_write(0x99a4, 0x1);
    }
        ...
        u16_t* mac, u32_t* key)
{
    ...
    cmd[2] = ((u32_t)zmw_le16_to_cpu(mac[0])<<16) + (u32_t)type;
    cmd[3] = ((u32_t)zmw_le16_to_cpu(mac[2])<<16) + ((u32_t)zmw_le16_to_cpu(mac[1]));
    
    /* 
     * If Big endian :
     * Key content although in little endian, convert it in big endian for wrapper.
     * Because wrapper would convert it by little endian.
     */
    for (i=0; i<4; i++)
    {
        cmd[4+i] = zmw_le32_to_cpu(key[i]);
    }
...
void zfInitMac(zdev_t* dev)
{
    ...
    zfDelayWriteInternalReg(dev, 0x1c3c44, 0x30);
        ...
        u16_t* aifsTbl, u16_t* txopTbl)
{
    ...
    {
#if 1 //Restore CWmin to improve down link throughput  
...
void zfHpLedCtrl(zdev_t* dev, u16_t ledId, u8_t mode)
{
    ...
    if (ledId < ZM_MAX_LED_NUMBER)
    {
        if (((struct zsHpPriv*)wd->hpPrivate)->ledMode[ledId] != mode)
        {
            ((struct zsHpPriv*)wd->hpPrivate)->ledMode[ledId] = mode;
            
            state = ((struct zsHpPriv*)wd->hpPrivate)->ledMode[0]   //GPIO 0
                    | (((struct zsHpPriv*)wd->hpPrivate)->ledMode[1]<<1); //GPIO 1
...
u8_t zfGetHwBtnExist(zdev_t* dev)
{
    zmw_get_wlan_dev(dev);
    ...
    //use futurebase[2] (EEPROM@0x1621) to indicate the WPS hardware push button existence or not.
    //check eepromImage 0x1621 is at 0x621/4
    ret = (u8_t)((hpPriv->eepromImage[0x621/4]>>8) & 0xff);
    
    //check if this position has been set to odd value
    if(ret != 1)
        ret = 0;
...
void zfSetPowerCalTable(zdev_t* dev, u32_t frequency, u8_t bw40, u8_t extOffset)
{
    ...
    {             
        ...
        /* 
         * Set Beacon power
         * REG_BCN_HT1 (0xDA0) bit 26:21 is Bcn TPC for Otus. 
         * Beacon phy rate in 2.4G is 1M and in 5G is 6M now.	 
         */
        zfDelayWriteInternalReg(dev, ZM_MAC_REG_BCN_HT1, (0x8000170 | 
                    ((hpPriv->tPow2xCck[0] & 0x2f) << 21)));
    ...
    else
    {
        ...
        #endif
        hpPriv->currentAckRtsTpc = hpPriv->tPow2x5g[0];
